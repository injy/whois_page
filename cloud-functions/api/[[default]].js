...
for (const [canonical, { aliases: aliases2 }] of Object.entries(STATUS_MAP)) {
...
function formatStatus(status) {
  for (const item of status) {
    ...
    const canonical = aliasToCanonical[key2];
    if (canonical) {
      item.text = canonical;
...
function shuffleWithWeight(entries2) {
  const scored = entries2.map((e) => ({
    entry: e,
...
function formatStatus2(status) {
  for (const item of status) {
    const key2 = item.text.replace(/[\s_]/g, "").toLowerCase();
    const canonical = aliasToCanonical2[key2];
    if (canonical) {
      item.text = canonical;
      if (!item.url || item.url === "https://icann.org/epp#active") {
...
${section}:
`;
      const textContent3 = content.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
      const parts = textContent3.split(/([A-Za-z\s-]+):/);
...
var hmScraper = async (domain) => {
  ...
  try {
    ...
    const setCookies = [];
    const rawSet = homeResponse.headers?.getSetCookie?.();
